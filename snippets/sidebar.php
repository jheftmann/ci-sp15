<div class="sidebar">
	<header>
		<h5><a href="http://jacobheftmann.com/ci-sp15/">
			Core Interaction
			<br>
			Spring 2015
			</a>
		</h5>
		<h5>
			Studio
			<br>
			PUCD 2125, 6576
			<br>
			Tues./Thurs. 1900&#8212;2140
			<br>
			Room TBD
			<br>
			Minnie Choi
			<br>
			<a href="mailto:choim1@newschool.edu">email</a>
		</h5>
		<h5>
			Lab
			<br>
			PUCD 2126, 6583
			<br>
			Mon. 1900&#8212;2140
			<br>
			Room 306
			<br>
			Jacob Heftmann
			<br>
			<a href="mailto:heftmanj@newschool.edu">email</a>
		</h5>
		<h5>
			TA
			<br>
			Betty Quinn
			<br>
			<a href="mailto:quine427@newschool.edu">email</a>
		</h5>
	</header>
	<nav>
		<ul>
			<li class="current"><span class="indicator"></span><a href="#description">Description</a></li>
			<li><span class="indicator"></span><a href="#calendar">Calendar</a></li>
			<li><span class="indicator"></span><a href="#projects">Projects</a></li>
			<li><span class="indicator"></span><a href="#resources">Resources</a></li>
			<li><span class="indicator"></span><a href="#grading">Grading</a></li>
			<li><span class="indicator"></span><a href="#policies">Policies</a></li>
		</ul>
		<ul class="subnav">
			<li><a href="http://www.jacobheftmann.com/ci-sp15/class-list.php">Class list</a></li>
			<li><a href="assets/Syllabus_Core_Interaction_SP15.pdf">Syllabus (PDF)</a></li>
			<li><a href="https://github.com/jheftmann/ci-sp15/tree/master/code-samples">Code samples</a></li>
		</ul>
	</nav>
</div>